...
- 検索ベース収集（search.list）は **07-21 実装済み**（別ワークフロー・3時間ごと）。ただし過去分の初回遡り（`--after`/`--before` の期間区切りバッチ）はまだ運用しておらず、随時少しずつ進める（詳細はセクション8）
...
| 検索収集は別ワークフロー・3時間ごと（collect.yml と同じ concurrency グループで直列化） | 直近分を素早く反映。6h収集とコスト/役割を分けつつ videos.json 書き込みは衝突させない | 07-21 |
...
方針は「登録チャンネル方式（collect.py・6h）＋検索ベース発見（search_collect.py・3h）」の併用で、**基本実装は完了**（07-21 push済み）。残るは過去分の初回遡り（下記2の期間区切りバッチ）の運用のみ。
...
- コスト: search.list=100ユニット/ページ（現行playlistItems=1の100倍）。3時間ごと×通常1ページ ≒ 800ユニット/日で上限1万に対し十分
