...
| 管理ツール①タブに絞り込みを追加（サイト側と同じ条件・PAGE_SIZE=50）。確認済みも編集対象にし「未確認に戻す」を用意 | 未確認1448件を全件描画していて手作業の修正時に目的の動画へ到達できなかった。既存の分類ミスを直すには確認済みも触れる必要がある | 07-25 |
| 管理ツールのキーワード検索は**タグも対象**に含める（サイト側は含めない） | チップに出ない自由タグ（第○回・イベント名）は検索でしか引けないため。閲覧用のサイトと違い、修正対象を名指しで呼び出す用途 | 07-25 |
| 一括確認は「表示中」ではなく**絞り込み結果の全件**が対象。件数表示＋2回クリックで実行 | 旧実装はラベルが「表示中」なのに未確認全件に効く不一致だった。誤爆の影響が大きいので確認を挟む | 07-25 |
| `[hidden]{display:none!important}` を index.html / admin.html の双方に追加 | `.more-btn{display:block}` が標準の `[hidden]` を打ち消し、「もっと見る」が古い残り件数のまま消えずに残っていた。`[hidden]` と `.more-btn` は詳細度が同じで記述順に負けるため `!important` で確定させる | 07-25 |
...
- 「未確認が大量に積まれる」運用負荷への対応（レビューUIの一括操作と絞り込みは実装済み。07-25 時点で未確認1448件のため、絞り込み単位で消化していけるかは運用してから判断）
